...
from keras.layers import Conv1D, GlobalAveragePooling1D, MaxPooling1D, TimeDistributed
...
EPOCHS = 5 #  an arbitrary cutoff, generally defined as "one pass over the entire dataset", used to separate training into distinct phases, which is useful for logging and periodic evaluation.
...
RNN_HIDDEN_DIM = 1 # Hidden Layers 
...
def create_lstm(input_length, X_train ,rnn_hidden_dim = RNN_HIDDEN_DIM, output_dim = OUTPUT_DIM, input_dim = INPUT_DIM, dropout = DROPOUT_RATIO):
    ...
    model.add(Conv1D(filters = 10, kernel_size = 3, activation='relu', input_shape=(input_length, 1)))
    model.add(MaxPooling1D(3))
    # model.add(Conv1D(filters = 10, kernel_size = 3, activation='relu'))
    # model.add(Conv1D(filters = 10, kernel_size = 3, activation='relu'))
    # model.add(GlobalAveragePooling1D())    
    model.add(Dropout(0.5))
    model.add(TimeDistributed(Flatten(input_shape=(input_length, 1))))
